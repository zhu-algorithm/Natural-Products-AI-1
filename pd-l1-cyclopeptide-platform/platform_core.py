...
class BindingSelectivityPredictor:
 def predict(self, seq):
  ...
  affinity=-5.2-2.8*h-a+.7*ch; selectivity=sigmoid(1.7*h+.7*a-1.4*ch-.15*abs(n-9))
  return {"affinity_kcal_mol":round(affinity,2),"selectivity_score":round(selectivity,3),"model":"sequence-composition surrogate"}
@dataclass
...
def run_pipeline(seed,count,weights):
 gen=CyclicPeptideGenerator(); bind=BindingSelectivityPredictor(); admet=ADMETSynthesizability(); dock=DockingModule(); rows=[]
 for i,item in enumerate(gen.generate(seed,count),1):
  seq=item["sequence"]; b=bind.predict(seq); rows.append({"id":f"CP-{i:03d}",**item,"binding":b,"admet":admet.assess(seq),"docking":dock.dock(seq,b["affinity_kcal_mol"])})
